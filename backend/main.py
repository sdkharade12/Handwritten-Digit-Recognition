...
def preprocess_digit_image_from_bytes(image_bytes: bytes) -> Tuple[Image.Image, np.ndarray]:
    ...
    # Center-of-mass alignment to mimic MNIST centering
    np_canvas = np.array(canvas)
    # Consider non-background pixels as foreground
    foreground_yx = np.argwhere(np_canvas > 0)
    if foreground_yx.size > 0:
        # Compute centroid (row=y, col=x)
        centroid_y, centroid_x = foreground_yx.mean(axis=0)
        shift_x = int(round(14 - centroid_x))
        shift_y = int(round(14 - centroid_y))
        if shift_x != 0 or shift_y != 0:
            canvas = ImageChops.offset(canvas, shift_x, shift_y)

    # Light blur can reduce spurs and improve model robustness
